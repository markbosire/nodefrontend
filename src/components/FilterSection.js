...
const FilterSection = ({ onFilterChange,selectedOptions ,setSelectedOptions,resetFilters}) => {
  const genres = [
  ...
  return (
    <div className='mt-3 filters'>
      <h3 style={{ fontSize: '1rem' }}>Filter ({selectedOptions.length}) <span onClick={resetFilters} className="reset-filters">Reset</span></h3>
      <Form.Control type="text" placeholder="Search by keyword" value={keyword} onChange={handleKeywordSearch} className='border-dark rounded-0 shadow-none text-dark outline-none'/>
      <br />
      <div className="filter-section" onClick={() => toggleSection('genre')}>
        <h3 style={{ fontSize: '1rem' }}>Genre <i className="dropdown-icon fa fa-chevron-down"></i></h3>
        <hr />
      ...
      <div className="filter-section" onClick={() => toggleSection('priceRange')}>
        <h3 style={{ fontSize: '1rem' }}>Price Range <i className="dropdown-icon fa fa-chevron-down"></i></h3>
        <hr />
        {openSection === 'priceRange' && (<div className="filter-options" style={{ maxHeight: '200px', overflowY: 'auto' }}>
          <div onClick={() => handleOptionClick('priceRange', 'free')} className="filter-option" style={{ fontSize: '0.9rem' }}>
            Free <i className={`check-icon fa ${selectedOptions.includes('priceRange:free') ? 'fa-check-square-o' : 'fa-square-o'}`}></i>
          </div>
          <div onClick={() => handleOptionClick('priceRange', '0-10')} className="filter-option" style={{ fontSize: '0.9rem' }}>
      ...
      <div className="filter-section" onClick={() => toggleSection('releaseDate')}>
        <h3 style={{ fontSize: '1rem' }}>Release Date <i className="dropdown-icon fa fa-chevron-down"></i></h3>
        <hr />
